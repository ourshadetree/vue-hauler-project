<template>
  <div class="station-map-container">
    <!-- Custom Layer Control (replacing default Google controls) -->
...
const lookupInput = ref(null)  // Ref for autocomplete input
...
// Create markers for each station with a global info window.
...
// Initialize the map, directions, and autocomplete.
async function initMap() {
  ...
  // Create the map with default UI disabled.
  ...
  // If the tools overlay is already visible, attach autocomplete.
  if (toolsExpanded.value && lookupInput.value) {
    initAutocomplete()
  }
}

// Function to initialize the Places Autocomplete on the lookup input.
function initAutocomplete() {
  const autocomplete = new google.maps.places.Autocomplete(lookupInput.value, {
    types: ['geocode']
  })
  autocomplete.addListener('place_changed', () => {
    ...
    if (place && place.geometry && place.geometry.location) {
      ...
      // Optionally update the v-model with the formatted address.
      lookupAddress.value = place.formatted_address || ''
    }
  })
}

// Watch for toolsExpanded changes to initialize autocomplete when the overlay becomes visible.
watch(toolsExpanded, async (newVal) => {
  if (newVal) {
    // Wait for the DOM update so that lookupInput is rendered.
    await nextTick()
...
// Address lookup (fallback): use geocodeAddress to recenter the map.
...
async function geocodeAddress(address) {
  const apiKey = 'AIzaSyDYpNJXRFRuQq5IV8LQZi8E90r1gIaiORI' // Your API key.
...
// Refresh the map: recenter, reset zoom, reload station data, re-create markers, clear route and info window.
