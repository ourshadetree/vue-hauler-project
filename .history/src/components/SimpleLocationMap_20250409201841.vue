...
// We also use "filteredStations" if needed for extra logic (here we filter markers directly)
...
// Toggle toolbox visibility and attach autocomplete to inputs.
...
// New Function 1: Filter Stations Near a Single Address
...
// New Function: Filter stations along the route using a bounding box approach.
function filterStationsByBoundingBox(pathArray) {
  // Calculate the bounding box from the route's path.
  let minLat = Infinity, maxLat = -Infinity, minLng = Infinity, maxLng = -Infinity;
  pathArray.forEach(pt => {
    // pt is a google.maps.LatLng object; use its lat() and lng() methods.
    const lat = pt.lat();
    const lng = pt.lng();
  ...
  // Optionally add a margin to the bounding box (e.g., 10% extra on each side).
  ...
  // Now, loop over each marker; show only if its position is inside the bounding box.
  markers.value.forEach((marker, idx) => {
...
// Handle the single address lookup: recenter map and filter stations near that address.
async function handleLookup() {
  ...
  if (location) {
    map.value.setCenter(location);
    map.value.setZoom(10);
    // Filter markers to only show those within 50 miles of the lookup address.
...
// Calculate a driving route between two addresses and then filter stations along the route.
async function calculateRoute() {
  ...
  directionsService.route(request, (result, status) => {
    if (status === 'OK') {
      ...
      // Get the overview polyline object.
      const polylineObj = result.routes[0].overview_polyline;
      if (!polylineObj) {
        console.error("No overview polyline found in the route result.");
        return;
      }
      // The polyline might be a plain string or an object; check and extract accordingly.
      const encoded =
        typeof polylineObj === 'string' ? polylineObj : polylineObj.points;
      if (!encoded) {
      ...
      // Use the new bounding box filter.
...
// Refresh the map: recenter, reset zoom, reload station data/markers, clear route and infoWindow.
