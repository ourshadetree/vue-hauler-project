...
// Optionally remove or modify this watcher if DOT search no longer affects the map.
watch(dotSearchQuery, async (query) => {
  if (query) {
    // If the DOT search should still remove current markers, you can keep:
    clearMarkers()
    // Now simply commit the DOT query to the store so that the navTools component can use it.
    store.commit('SET_DOT_SEARCH_QUERY', query)
    
    // Clear any map-specific commands if needed.
    store.commit('CLEAR_MAP_COMMAND')
    
    // Optionally, you can clear the DOT search query here or let the navTools component handle it.
