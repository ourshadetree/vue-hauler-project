<template>
  <div class="mapContainer">
    ...
    <!-- Styled nav bar with two buttons and search results -->
    <div id="mapTools" class="tools-nav">
      <div class="tools-links">
        ...
        <!-- DOT search results appear next to the buttons -->
        <div
...
export default {
  ...
  setup() {
    ...
    // 2) We store ALL markers (red or blue) here.
    const currentMarkers = ref([])

    // 3) DOT search results to display in the nav bar
    ...
    // Geocode something like "City, State, USA" => lat/lng
    ...
    // Re-center map, re-load the aggregated states, clear any search results
    async function refreshMap() {
      ...
      // Clear DOT results from the toolbar
      dotSearchResults.value = []
    }

    // Called once -> create map -> load aggregated states
    ...
    async function dotSearch() {
      ...
      // Query potential_users for company_name, city, state
      const { data, error } = await supabase
      ...
      // For each result => geocode city,state => place a BLUE marker
      for (const row of data) {
      ...
      // If exactly 1 result, we can zoom into it
    ...
    onMounted(() => {
      console.log('[onMounted] Checking for window.google.maps...')
