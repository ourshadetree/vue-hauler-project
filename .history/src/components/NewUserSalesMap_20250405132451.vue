...
// Marker for current members: using the huIcon.png asset
import huIcon from '@/assets/huIcon.png'

// Red marker creation is no longer needed since the base markers now use huIcon
...
export default {
  ...
  setup() {
    const map = ref(null)
    const currentMarkers = ref([])       // Tracks all markers on the map
    const dotSearchResults = ref([])     // DOT search results for display
    ...
    async function loadMemberMarkers() {
      ...
      for (const row of data) {
        ...
        // Create a marker using the huIcon.png asset
    ...
    // Refresh the map: clear markers, reset center/zoom, clear DOT search results, and reload current member markers.
    ...
    async function initMap() {
      ...
      console.log('[initMap] Map created. Loading current member markers...')
      await loadMemberMarkers()
    }

    // DOT search: query potential_users for matching dot_number,
    // clear current markers, then place BLUE markers based on (city, state)
    async function dotSearch() {
      ...
      // Query potential_users, selecting business_name, city, state
      const { data, error } = await supabase
      ...
      // Use LatLngBounds to zoom into the results if applicable
      ...
      for (const row of data) {
        ...
        const marker = createBlueBubbleMarker(
          position,
          row.business_name, // label text uses business_name
