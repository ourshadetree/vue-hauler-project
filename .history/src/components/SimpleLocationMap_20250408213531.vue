...
function filterStationsAlong(path) {
  // build a Polyline object just for testing
  const routeLine = new google.maps.Polyline({ path })

  markers.value.forEach((marker, idx) => {
    const station = stations.value[idx]
    const point = new google.maps.LatLng(station.lat, station.lng)
    const onRoute = google.maps.geometry.poly.isLocationOnEdge(
      point,
      routeLine,
      0.1
    )
    console.log('Filtering stations along path:', idx, onRoute)
