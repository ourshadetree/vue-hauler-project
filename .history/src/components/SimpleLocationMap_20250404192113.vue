...
export default {
  ...
  setup() {
    ...
    // Generates a default marker icon URL with a specific color.
    const getMarkerIconUrl = (colorHex) => {
      // Remove the '#' character if present.
      const cleanColor = colorHex.replace('#', '')
      return `https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|${cleanColor}`
    }
    ...
    const loadStations = async (tables) => {
      ...
      allData.forEach((station) => {
        if (station.latitude && station.longitude && station.table) {
          ...
          const iconUrl = getMarkerIconUrl(markerColor)
          const marker = new google.maps.Marker({
            ...
            icon: {
              url: iconUrl,
              // You can optionally adjust the size by adding scaledSize property:
