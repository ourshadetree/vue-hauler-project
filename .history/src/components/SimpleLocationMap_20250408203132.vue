  ...
  <div class="station-map-container">
    ...
    <div v-if="toolsExpanded" class="map-overlay">
      <div class="controls-wrapper">
        <!-- Single address lookup input with autocomplete -->
        ...
        <!-- Routing Inputs: Now text inputs instead of selectors -->
        <div class="control-group">
          <input
            type="text"
            v-model="fromAddress"
            ref="fromInput"
            placeholder="From: Enter starting address"
        ...
        <div class="control-group">
          <input
            ...
            v-model="toAddress"
            ref="toInput"
            placeholder="To: Enter ending address"
            @keyup.enter="calculateRoute"
          />
        </div>
        <div class="control-group">
...
const map = ref(null)
const markers = ref([])
const stations = ref([]) // Array for station data
...
// New routing inputs (for user-entered addresses)
...
// Toolbox expanded state (collapsed by default)
...
// Create markers for each station, attaching one global info window.
...
// Initialize the map, directions, and autocomplete for the lookup input.
async function initMap() {
  ...
  // Initialize autocomplete on the lookup input if visible.
  if (toolsExpanded.value && lookupInput.value) {
    initAutocomplete(lookupInput.value)
  }
  // Also initialize autocomplete for routing inputs.
  if (toolsExpanded.value && fromInput.value) {
    new google.maps.places.Autocomplete(fromInput.value, { types: ['geocode'] })
  }
  if (toolsExpanded.value && toInput.value) {
    new google.maps.places.Autocomplete(toInput.value, { types: ['geocode'] })
  }
}
...
function initAutocomplete(inputElement) {
  ...
  autocomplete.addListener('place_changed', () => {
    ...
    if (place && place.geometry && place.geometry.location) {
      ...
      lookupAddress.value = place.formatted_address || ''
    }
  })
}

// Watch for tools overlay expansion to attach autocomplete to routing inputs.
watch(toolsExpanded, async (newVal) => {
  if (newVal) {
    await nextTick()
...
// Address lookup: recenter the map using geocode result.
...
// Calculate a driving route using the "From" and "To" address inputs.
