...
function fetchAndCreateMarkers(stationData) {
  ...
  stationData.forEach(station => {
    ...
    // inline everything so *every* marker opens an InfoWindow
    m.addListener('click', () => {
      // 1) track active ID (so your list highlights)
      ...
      // 3) open InfoWindow with this station’s info
      const content = `
...
function handleStationSelect(stationId) {
  ...
  fetchAndCreateMarkers(nearbyStations.value)
}
