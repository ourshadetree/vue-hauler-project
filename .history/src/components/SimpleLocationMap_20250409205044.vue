...
// (Optional) A reactive array if you want to store filtered stations separately.
...
function createMarkers() {
  ...
  stations.value.forEach(station => {
    ...
    // Attach the station data directly to the marker:
...
/*
  ...
  We use computeDistanceBetween to check each station’s distance from the lookup location.
*/
...
markers.value.forEach((marker) => {
  const station = marker.stationData; // or stations.value[idx], if that’s how you coded it
...
/*
  New Function 2: Filter stations along the route using a bounding box approach.
  We compute the min/max latitude and longitude from the route's decoded polyline and only show markers that lie within that box.
*/
function filterStationsByBoundingBox(pathArray) {
  // Compute bounding box over the route:
  ...
  // Add a little margin (10% of the box dimensions).
...
// Handle single address lookup: recenter map and filter stations near that address.
...
// Calculate a driving route between two addresses and filter stations along the route using the bounding box approach.
...
// Refresh the map: recenter, reset zoom, reload station data/markers, clear route and close infoWindow.
