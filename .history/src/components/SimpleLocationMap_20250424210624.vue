<template>
  <div class="station-map-container">
    ...
    <div v-if="nearbyStations.length" class="station-list-container">
  <div class="station-list-header">
    <h3>Nearby Stations</h3>
    <button @click="collapsed = !collapsed" class="collapse-button">
  ...
  <div v-show="!collapsed" class="station-list-content">
    <ul>
      <li
        ...
        @click="handleStationSelect(st.id)"
      >
        <strong>{{ st.brand_name || st.station_name }}</strong>
        – {{ st.distance || '' }}
      </li>
    </ul>
  </div>
...
const reference = ref({ lat: 0, lng: 0 })
...
function fetchAndCreateMarkers(stationData) {
  ...
  stationData.forEach(station => {
    ...
    // when marker clicked, use our select helper
    m.addListener('click', () => handleStationSelect(station.id))
