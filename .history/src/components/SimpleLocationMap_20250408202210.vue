...
// Reference for the lookup input (for Places Autocomplete)
...
async function initMap() {
  ...
  // Close info window when clicking outside markers.
  ...
  // Initialize Places Autocomplete on the lookup input.
  if (lookupInputRef.value) {
    const autocomplete = new google.maps.places.Autocomplete(lookupInputRef.value, {
      types: ['geocode']
    });
    autocomplete.addListener('place_changed', () => {
      ...
      if (place && place.geometry && place.geometry.location) {
        ...
        map.value.setCenter(locObj);
        map.value.setZoom(10);
        // Optionally, update the v-model:
...
// Refresh the map: recenter, reset zoom, reload station data and markers, clear routes and close info window.
...
/* Expand button when tools are collapsed */
.expand-button {
  position: absolute;
  top: 10px;
  left: 10px;
