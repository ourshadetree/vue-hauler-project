...
function refreshMap() {
    ...
    // 1) Clear route inputs and directions
    this.fromAddress = '';
    this.toAddress = '';
    this.lookupAddress = '';
    ...
    this.markers.forEach((m, idx) => {
      console.log('Removing marker:', idx, m.getTitle());
      m.setMap(null);
    });
    this.markers = [];

    // 3) Reset the map to its default center/zoom.
    // Option A: destroy and reinitialize the map, e.g.
    this.map = null;
    this.initMap();

    // 4) Clear the nearby stations data so that the overlay unmounts.
    this.filteredStations = []; // This will cause v-if (filteredStations.length > 0) to be false.
