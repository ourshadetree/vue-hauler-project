...
async function signIn() {
  console.log('🔐 signIn start:', email.value)
  errorMessage.value = ''
  loading.value = true

  // 1) Grab the promise (don’t confuse this with the function itself!)
  const p = supabase.auth.signInWithPassword({
    email:    email.value,
    password: password.value,
  })
  console.log('🎁 got promise:', p, '– has .then?', typeof p.then === 'function')

  // 2) Attach explicit handlers so you’ll see *something* even if `await` never continues:
  p.then(res => console.log('✅ promise resolved:', res))
   .catch(err => console.error('❌ promise rejected:', err))

  try {
    // 3) Now await the same promise
    const { data, error } = await p
    console.log('🎯 post-await result:', { data, error })
