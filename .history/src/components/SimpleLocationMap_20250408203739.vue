...
function toggleTools() {
  ...
  // When expanding, wait for the DOM to update and initialize autocomplete
  if (toolsExpanded.value) {
    nextTick(() => {
      if (lookupInput.value) initAutocomplete(lookupInput.value)
      if (fromInput.value) new google.maps.places.Autocomplete(fromInput.value, { types: ['geocode'] })
      if (toInput.value) new google.maps.places.Autocomplete(toInput.value, { types: ['geocode'] })
    })
  }
}
...
// Create markers (info windows show brand_name, address, city, state)
...
async function initMap() {
  ...
  // Initialize autocomplete for lookup and routing inputs if the tools overlay is visible.
  await nextTick()
...
// Clear route input fields and any displayed directions.
