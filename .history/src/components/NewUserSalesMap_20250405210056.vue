...
// Import the current member icon asset (huIcon.png)
...
export default {
  ...
  setup() {
    ...
    // Helper: geocode an address string to get a LatLng
    ...
    // Load base markers from the current_members table
    async function loadMemberMarkers() {
      ...
      for (const row of data) {
        ...
        // Create a marker using huIcon.png for current members
    ...
    // Load DOT search markers from the potential_users table
    async function loadDotSearchMarkers(dotNumber) {
      console.log(`[loadDotSearchMarkers] Searching for DOT: ${dotNumber}`)
      // Query potential_users for rows matching the given dot_number
      const { data, error } = await supabase
      ...
      // Save results to Vuex so the nav tools component can display them
      ...
      // Use a LatLngBounds to adjust the map view
      ...
      for (const row of data) {
        ...
        if (!position) continue
        // Create a blue marker for the DOT search result
        const marker = new google.maps.Marker({
          position,
          map: map.value,
    ...
    // Refresh the map: clear markers, reset map view, and reload member markers
    async function refreshMap() {
      console.log('[refreshMap] Refreshing map to default state markers...')
      if (!map.value) return
      clearMarkers()
    ...
    // Initialize the map and load the member markers
    ...
    // Watch Vuex store for commands and DOT search queries
