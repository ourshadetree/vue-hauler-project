<template>
  ...
  const emit  = defineEmits(['update:profile','valid','edit-address'])
  
  const local = reactive({
    shipping_option: props.profile.shipping_option || 'standard'
  })
  
  watch(local, () => {
    emit('update:profile', { ...props.profile, ...local })
    emit('valid', !!local.shipping_option)
  ...
  .address { margin:0.5rem 0; font-weight:500 }
  a { display:inline-block; margin-bottom:1.5rem; color:#646cff; text-decoration:underline }
  .checkbox-group { display:flex; flex-direction:column; gap:1rem; margin-top:1rem }
  .checkbox-option { display:flex; align-items:center }
  .checkbox-option input { margin-right:0.5rem }
  </style>
