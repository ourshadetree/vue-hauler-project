<template>
    <div class="step-wrapper">
      ...
      <div class="field">
        ...
        <input id="cardholder_name" v-model="local.cardholder_name" type="text"/>
      </div>
      <div class="field">
        <label for="card_number">Card Number</label>
        <input id="card_number" v-model="local.card_number" type="text" maxlength="19"/>
      </div>
      <div class="field">
        <label for="expiry_date">Expiry Date</label>
        <input id="expiry_date" v-model="local.expiry_date" type="month"/>
      </div>
    </div>
  </template>
  ...
  const local = reactive({
    cardholder_name: props.profile.cardholder_name || '',
    card_number:     props.profile.card_number     || '',
    expiry_date:     props.profile.expiry_date     || ''
  })
  
  watch(local, () => {
    emit('update:profile', { ...props.profile, ...local })
    const valid = ['cardholder_name','card_number','expiry_date']
